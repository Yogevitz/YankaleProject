...
class ReadFile:
            ...
            for doc_contents in file.split("</DOC>\n\n<DOC>"):
                ...
                if '</TEXT>' in doc_contents:
                    doc_text = doc_contents.split('<TEXT>')[1].split('</TEXT>')[0].replace('\n', ' ') \
                        ...
                        .replace('^', '').replace('\ ', '').replace('//', '').replace('@', '') \
                        .replace('   ', ' ').replace('  ', ' ')
                self.docs_properties[doc_name] = {'language': doc_language, 'city': doc_city.upper()}
    ...
    #               value = { key = doc_name
    #                         value = [ indexes ] }
    def read_city_language(self):
        for file_name in self.file_names_split[self.file_names_split_index]:
            file = open(self.root_folder_path + file_name + "/" + file_name, "r").read()
            # for doc_contents in file.split("</DOC>"):
            for doc_contents in file.split("<DOC>")[1].split("<F>")[0]:
                doc_language = ''
                doc_city = ''
                doc_name = ''
                if doc_contents != '\n':
...
class Parse:
    ...
    def word_parser(self, tokens, token, index):
        ...
        if token == 'between' or token == 'Between':
            if (index < len(tokens) - 1 and index < len(tokens) - 2 and tokens[index + 1][0].isdigit()
                    ...
                    and index < len(tokens) - 4 and tokens[index + 1] != '' and tokens[index + 3][0].isdigit()):
    ...
    def save_doc_data(self, doc_name, doc_terms, doc_properties, max_tf):
        ...
        for term in doc_terms:
            if term.upper() in self.terms_dictionary:
                ...
                if 'tf_per_doc' not in self.terms_dictionary[term].keys():
                    self.terms_dictionary[term]['tf_per_doc'] = {doc_name: doc_terms[term]['tf']}
                else:
                    tmp_dict = {doc_name: doc_terms[term]['tf']}
                    self.terms_dictionary[term]['tf_per_doc'].update(tmp_dict)
    ...
    def save_parser_data(self):
        ...
        for key in sorted(self.terms_dictionary.keys()):
            ...
            str_docs_list = str(self.terms_dictionary[key]['docs']).replace(' ', '')
            terms_file.write("<" + key + "," + str_df + "," + str_tf_per_doc + "," + str_docs_list + ">\n")
        for doc in sorted(self.docs_dictionary.keys()):
            ...
            docs_file.write("<" + doc + "," + str_max_tf + ","
...
def concurrent_function(rf, sw_path, index, lock):
    (docs_texts, docs_properties) = rf.read(lock)
    parser = Parse(index, sw_path, stemming_bool)
...
if __name__ == '__main__':
    ...
    for thread_index in range(number_of_threads):
        process = Process(target=concurrent_function, args=[read_file, stop_words_path, thread_index, lock])
        process.start()
